...
if __name__ == "__main__":
    ...
    CoT_max =   90    ## maximum sampling time of CoT    
    L2M_max =   75    ## maximum sampling time of L2M
    SBP_max =   25    ## maximum sampling time of SBP       
    AnP_max =   40    ## maximum sampling time of AnP
    ...
    path = os.path.join(log_path, args.dataset, f"{args.dataset}_cost.xlsx")
    ...
    try:    
        ...
        assert counter["DiP"] >= DiP_max
        assert counter["CoT"] >= CoT_max
        assert counter["L2M"] >= L2M_max
        assert counter["SBP"] >= SBP_max
        assert counter["AnP"] >= AnP_max
        # assert counter["ToT"] >= 3
        # assert counter["S-RF"] >= S_RF_max
        # assert counter["MAD"] >= MAD_max
    except:
        ...
        for reasoning in reasonings:
            try:
                if reasoning == "DiP":
                    ...
                    args.reasoning = "CoT"
                    args.shot = 0
                    for N in range(0, CoT_max+1):
                        if not os.path.exists(os.path.join(log_path, args.dataset, args.model_name, f"{args.reasoning}_{args.shot}_{N}.json")):
                            break
                        
                elif "L2M" in reasoning:
                    args.reasoning = "L2M"
                    if args.dataset in ["GSM8K", "GSM-Hard"]:
                        args.shot = 1
                    else:
                        args.shot = 0
                    for N in range(0, L2M_max+1):
                        if not os.path.exists(os.path.join(log_path, args.dataset, args.model_name, f"{args.reasoning}_{args.shot}_{N}.json")):
                            break
                    ...
                    args.shot = 0
                    N = min((len(logs[-1]["record"]) - 1) // 2, S_RF_max)
                    
                elif reasoning == "SBP":
                    args.reasoning = "SBP"
                    args.shot = 0
                    for N in range(0, SBP_max+1):
                        if not os.path.exists(os.path.join(log_path, args.dataset, args.model_name, f"{args.reasoning}_{args.shot}_{N}.json")):
                            break
                    ...
                    args.n = 0
                    logs = read_logs(args)
                    N = min(len(logs[0]["record"]), MAD_max)
                ...
                for m in range(0, N):
                    ...
                    cost = get_cost(args.model_name, prompt_tokens_, completion_tokens_)
    ...
    max_cost = max(cost) // 0.1 * 0.1
    x_ticks = [max_cost / 10 * i for i in range(1, 11)]
    pos = - x_ticks[0] / 1
    ...
    plt.axhline(y=y_max + ind_line * 0.5, color='k', linestyle='-', alpha=0.5, zorder=10, linewidth=1.5)
    ...
    plt.text(pos, y_max + ind_line,  r'$\mathbf{P}_{O}^*$', fontsize=12, ha='center', va='center')  
    plt.rcParams['font.weight'] = 'bold'
    plt.rcParams['axes.labelweight'] = 'bold'
    plt.rcParams['axes.titleweight'] = 'bold'
    ...
    plt.savefig(os.path.join(log_path, args.dataset, args.model_name, "pics", f"Performance_cost.png"), bbox_inches='tight', dpi = 600)
    plt.close()
    print(f"Performance_cost.png saved to {os.path.join(log_path, args.dataset, args.model_name, 'pics', 'Performance_cost.png')}")
